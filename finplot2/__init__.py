...
# Maybe look into a python auto-formatter (set tabsize=4)
# Can we fix the missing functions from parents - due to hacks of importing them
#   down?
...
class FinWindow(pg.GraphicsLayoutWidget):
    ...
    def resizeEvent(self, ev: Optional[QtGui.QResizeEvent]) -> None:
        ...
        if ev and not self.closing:
            ...
            client_borders = old_win_height - sum(ax.vb.size().height() for ax in self.axs if ax.vb is not None)
            ...
            for i,ax in enumerate(self.axs):
                ...
                ax.setMinimumSize(100 if axis_height_factor[i] > 1 else 50, new_axes_height*f)
...
def _create_timestamp_plot(prev_ax: pg.PlotItem | None, viewbox: FinViewBox, index: int) -> pg.PlotItem:
